...
//    the code doesn`t work ther is a problem the retrofit doesn`t read inside photo
...
//        rv.layoutManager= LinearLayoutManager(this)
    ...
    fun search() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            if (apif != null) {
                apif.getdat("flickr.photos.search","33a9d39f0edb9d9c75cdf2a50fa983cd",ed.text.toString(),sell[sel].toString(),"xmlrpc",)
                    .enqueue(object : Callback<MethodResponse?> {
                        override fun onResponse(call: Call<MethodResponse?>, response: Response<MethodResponse?>
                            ...
                            Log.d("responimg", "onResponse: ${response.body().toString()}")
                            Log.d("resimg", "onResponse: ${response.body()?.params?.param?.value?.string?.photos?.photo}")
                            var a = response.body()?.params?.param?.value?.string?.photos
                            if (a != null) {
//                                for (i in a.params?.param?.value?.string?.photos) {
//
//                                    image.imgarr.add(
//                                        image(
//                                            i.title,
